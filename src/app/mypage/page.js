...
export default function MyPage() {
  ...
  if (tier === 'pro') {
    ...
    maxLimit = Infinity;
    tierName = "👑 ADMIN (무제한)";
    tierColor = "bg-gray-800 text-white";
  }

  // 퍼센트 계산 (admin은 항상 0%로 표시해서 여유 있게 보이게 함)
    ...
    <div className="min-h-screen bg-gray-50 p-4 md:p-8 font-sans">
      <div className="max-w-5xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          ...
          <button onClick={() => router.push('/')} className="bg-white border px-4 py-2 rounded-lg font-bold hover:bg-gray-50">← 홈으로</button>
        </div>
          ...
          <div className="md:col-span-1 space-y-6">
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
              ...
              <div className="mb-6">
                ...
                <div className={`mt-2 inline-block px-2 py-1 text-xs font-bold rounded ${tierColor}`}>
                  {tierName}
                </div>
              </div>
              ...
              <div className="mb-8 bg-gray-50 p-4 rounded-xl border border-gray-200">
                ...
                <div className="w-full bg-gray-200 rounded-full h-2.5 overflow-hidden">
                  <div className={`h-2.5 rounded-full transition-all duration-500 ${percentage >= 100 ? 'bg-red-500' : 'bg-blue-600'}`} style={{ width: `${percentage}%` }}></div>
                </div>
                <p className="text-xs text-gray-400 mt-2 text-center">
                  {tier === 'admin' ? "관리자는 무제한 생성 가능합니다." : (percentage >= 100 ? "한도 초과" : "아직 여유가 있어요!")}
                </p>
              </div>

              <button onClick={handleLogout} className="w-full border border-gray-300 text-gray-500 py-2 rounded-lg text-sm hover:bg-gray-100 font-bold">로그아웃</button>
            </div>
          </div>
          ...
          <div className="md:col-span-2">
            ...
            {myTrips.length === 0 ? (
              <div className="bg-white p-12 rounded-2xl shadow-sm text-center border border-dashed border-gray-300">
                ...
                <button onClick={() => router.push('/')} className="text-blue-600 font-bold underline">여행 만들기</button>
              </div>
            ) : (
                ...
                {myTrips.map((trip) => (
                  <div key={trip.id} className="bg-white p-5 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition flex justify-between items-center">
                    <div>
                      ...
                      <div className="text-sm text-gray-500 flex gap-3"><span>📍 {trip.destination}</span><span>🗓️ {trip.duration}</span></div>
                      <p className="text-xs text-gray-400 mt-2">{new Date(trip.created_at).toLocaleDateString()} 생성됨</p>
                    </div>
                    <button onClick={() => handleDelete(trip.id)} className="text-red-400 hover:text-red-600 text-sm px-3 py-2 bg-red-50 rounded-lg font-bold transition">삭제</button>
                  </div>
                ))}
              </div>
            )}
