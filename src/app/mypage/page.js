...
export default function MyPage() {
  ...
  const handleAvatarUpload = async (event) => {
    ...
    } catch (error) { alert('업로드 실패: ' + error.message); } finally { setUploading(false); }
  };
  ...
  const handleShare = (e, tripId) => {
    ...
    navigator.clipboard.writeText(shareUrl).then(() => alert("링크 복사 완료! 🔗")).catch(() => alert("복사 실패"));
  };
  ...
  let maxLimit = 3; let tierName = "Free Plan"; let badgeColor = "bg-slate-100 text-slate-600";
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-slate-800">
      ...
      <main className="max-w-6xl mx-auto px-6 py-12">
        <div className="bg-white rounded-3xl border border-slate-200 p-8 mb-12 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            ...
            <div className="relative group">
              <label htmlFor="avatar-upload" className="cursor-pointer">
                <div className="w-24 h-24 rounded-full overflow-hidden border-4 border-white shadow-md bg-slate-100 flex items-center justify-center relative">
                  {uploading ? <span className="text-xs font-bold text-slate-400">UP..</span> : avatarUrl ? <img src={avatarUrl} alt="Profile" className="w-full h-full object-cover" /> : <span className="text-4xl">👤</span>}
                  <div className="absolute inset-0 bg-black/30 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"><span className="text-xl">📷</span></div>
                </div>
              </label>
              ...
              <div className="flex items-center gap-3 mb-1 min-h-[36px]">
                {isEditing ? (
                  <div className="flex items-center gap-2 animate-fade-in">
                    <input type="text" value={newNickname} onChange={(e) => setNewNickname(e.target.value)} className="border border-slate-300 rounded-lg px-3 py-1 text-lg font-bold text-slate-900 outline-none focus:border-rose-500 w-40" autoFocus />
                    ...
                    <button onClick={() => { setIsEditing(false); setNewNickname(nickname); }} className="w-8 h-8 flex items-center justify-center bg-slate-200 text-slate-600 rounded-full hover:bg-slate-300 transition">✕</button>
                  </div>
                ) : (
                  <div className="flex items-center gap-2 group">
                    <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2">안녕하세요, <span className="text-rose-500">{nickname}</span>님!</h1>
                    <button onClick={() => setIsEditing(true)} className="text-slate-400 hover:text-rose-500 transition opacity-0 group-hover:opacity-100 bg-slate-50 p-1 rounded-full">✏️</button>
                  </div>
                )}
              </div>
          ...
          <div className="w-full md:w-auto flex flex-col items-end gap-4">
             <div className="bg-slate-50 px-6 py-3 rounded-xl border border-slate-100 w-full md:w-64">
                <div className="flex justify-between text-xs font-bold text-slate-500 mb-2"><span>이번 달 생성</span><span>{limitInfo?.usage_count} / {tier === 'admin' ? '∞' : maxLimit}</span></div>
                <div className="w-full bg-slate-200 rounded-full h-2 overflow-hidden"><div className="bg-rose-500 h-2 rounded-full transition-all duration-500" style={{ width: `${percentage}%` }}></div></div>
             </div>
             <button onClick={handleLogout} className="text-sm font-bold text-slate-400 hover:text-slate-800 underline decoration-2 underline-offset-4 transition">로그아웃</button>
          </div>
        </div>
        ...
        <div className="mt-24 pt-10 border-t border-slate-100 flex justify-center"><button onClick={handleWithdrawal} className="text-xs text-slate-400 hover:text-red-500 hover:underline transition">회원 탈퇴하기</button></div>
      </main>
    </div>
  );
}