...
export default function SharedTripPage() {
  ...
  // ✨ [핵심] 지도 URL 생성 로직 (메인 페이지와 동일하게 업그레이드)
  const getMapUrl = (activities) => {
    if (!activities || activities.length === 0) return null;

    // 1. 사용자가 카드를 클릭했다면 -> 그 장소만 보여주는 'Place Mode'
    if (selectedActivity) {
    ...
    // 2. 선택된 게 없다면 -> 전체 경로 'Directions Mode'
        ...
        <div className="animate-fade-in-up pb-20">
          ...
          <div className="bg-white p-2 mb-8">
            ...
            <div className="flex flex-wrap items-center gap-3 text-sm font-medium text-slate-500">
              ...
              <span className="bg-slate-50 px-3 py-1 rounded-lg border border-slate-100 flex items-center gap-1"><span className="text-rose-500">👥</span> {trip.companions}</span>
            </div>
          </div>
                ...
                {currentDayPlan.activities.map((act, idx) => (
                  <div key={idx} className="ml-8 relative group">
                    ...
                    <div 
                        ...
                        className={`bg-white rounded-2xl border overflow-hidden hover:shadow-xl transition-all duration-300 cursor-pointer group ${selectedActivity === act ? 'border-rose-500 ring-2 ring-rose-100' : 'border-slate-200'}`}
                    >
                       <div className="flex">
                         {/* 이미지 */}
                         ...
                         <div className="p-4 flex-1">
                           <div className="flex justify-between items-start mb-1">
                              <h3 className="font-bold text-lg text-slate-900 leading-tight line-clamp-1">{act.place_name}</h3>
                           ...
                           <div className="flex flex-wrap gap-2">
                             ...
                             {act.googleMapsUri && (
                               <a 
                                 ...
                                 onClick={(e) => e.stopPropagation()} // 카드 클릭(지도이동) 방지
                                 className="text-xs font-bold text-slate-600 bg-slate-100 px-3 py-1.5 rounded-lg hover:bg-slate-200 transition flex items-center gap-1"
                               >
                                 🗺️ 구글맵
                               </a>
                             )}
                             ...
                             {act.booking_url && (
                               <a 
                                 ...
                                 className="text-xs font-bold text-rose-500 bg-rose-50 px-3 py-1.5 rounded-lg hover:bg-rose-100 transition flex items-center gap-1"
                               >
                                 🎟️ 예약하기
                               </a>
                             )}
              ...
              <div className="sticky top-24 h-[calc(100vh-150px)] min-h-[500px] bg-slate-100 rounded-[2rem] shadow-inner border border-slate-200 overflow-hidden relative">
                {getMapUrl(currentDayPlan.activities) ? (
                  <>
                    ...
                    {/* 전체 경로 보기 버튼 (장소 선택 시에만 등장) */}
                    {selectedActivity && (
                       <button 
                         onClick={() => setSelectedActivity(null)}
                         className="absolute top-4 left-4 bg-white/90 backdrop-blur text-slate-800 px-4 py-2 rounded-full shadow-lg text-sm font-bold hover:bg-white transition flex items-center gap-2"
                       >
                         🔙 전체 경로 보기
                       </button>
                    )}
